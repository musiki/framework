...
export const createLiveKitTokenResponse = async ({ request, locals }: APIContext) => {
  try {
    ...
    if (!apiKey || !apiSecret) {
      console.error('[LIVEKIT-TOKEN] API Key or Secret missing in environment');
    ...
    console.error('[LIVEKIT-TOKEN] Fatal error generating token:', error);
    return json({ error: 'Internal server error while generating token', details: error.message }, 500);
  }
};
