...
Este flujo mantiene tu frontend Astro  y tu capa de IA en VPS con Ollama.

## 1. Arquitectura recomendada

- VPS Astro: UI + rutas públicas.
- VPS Ubuntu 23: `ollama serve` + API Fastify (`services/ollama-api`).
- Ollama solo accesible por `localhost:11434` en el VPS.
- Fastify expuesto por `Caddy` en `api.tu-dominio.com`.
...
```bash
ssh zz@46.225.154.68 'sudo systemctl restart ollama-correction-api'
```

## 4. Exponer API con Caddy
...
```bash
ssh zz@46.225.154.68 'journalctl -u ollama-correction-api -f'
```

Estado:

```bash
ssh zz@46.225.154.68 'systemctl status ollama-correction-api --no-pager'
```
